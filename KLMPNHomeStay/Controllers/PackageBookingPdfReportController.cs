...
namespace KLMPNHomeStay.Controllers
{
    ...
    [ApiController]
    public class PackageBookingPdfReportController : Controller
    {
        ...
        public PackageBookingPdfReportController(klmpnhomestay_dbContext context, IWebHostEnvironment env, IConfiguration configuration, IGlobalService globalService)
        {
            _context = context;
            _env = env;
        ...
        public async Task<IActionResult> pdfExport()
        {
            ...
            try
            {
                ...
                var BookingList = await PackageBoolingList.OrderByDescending(m=>m.BookingDate).ToListAsync();
        ...
        private void ReportHeader(int? TotalCost)
        {
            ...
            _fontStyle = FontFactory.GetFont("Tahoma", 11f, 1);
            _pdfPCell = new PdfPCell(new Phrase("List Of Package Booking Details", _fontStyle));
            _pdfPCell.Colspan = _totalColumn;
