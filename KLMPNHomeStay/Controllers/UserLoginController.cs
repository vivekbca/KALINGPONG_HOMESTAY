...
namespace KLMPNHomeStay.Controllers
{
    ...
    public class UserLoginController : Controller
    {
        ...
        public UserLoginController(klmpnhomestay_dbContext context, IWebHostEnvironment env, IConfiguration configuration, IGlobalService globalService)
        {
            _context = context;
            _env = env;
        ...
        [HttpPost]
        public async Task<IActionResult> UserLoginCheck([FromBody] UserLoginResponseModel userLogin)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        var adminRole = await _context.TmUserRole.Where(m => m.RoleName == String.Format("Admin")).FirstOrDefaultAsync();
                        var logincheck = await _context.TmUser.Where(m => m.UserEmailId == userLogin.UserName && m.UserPassword == userLogin.Password).FirstOrDefaultAsync();
                        if (logincheck != null && logincheck.UserRoleId == adminRole.RoleId)
                        {
