...
namespace KLMPNHomeStay.Controllers
{
    ...
    [ApiController]
    public class PackageCancelPdfReportController : Controller
    {
        ...
        public PackageCancelPdfReportController(klmpnhomestay_dbContext context, IWebHostEnvironment env, IConfiguration configuration, IGlobalService globalService)
        {
            _context = context;
            _env = env;
        ...
        public async Task<IActionResult> pdfExport()
        {
            ...
            try
            {
                ...
                var BookingList = await PackageBoolingList.OrderByDescending(m=>m.CancelledDate).ToListAsync();
                foreach (var item in BookingList)
                {
                    HsBookingPdfModel obj = new HsBookingPdfModel();
                    obj.GuId = item.GuId;
                    obj.BkDateFrom = item.FromDate;
                    obj.GuName = item.GuName;
                    obj.BkDateTo = item.ToDate;
                    obj.BkNoPers = item.NoOfPerson;
                    obj.HsName = item.Name;
                    obj.BkBookingDate = item.BookingDate;
                    obj.BkCancelledDate = Convert.ToDateTime(item.CancelledDate);
                    obj.TotalCost = item.TotalRate;
                    model.Add(obj);
                }
            }
            catch (Exception ex)
        ...
        private void ReportHeader(int? TotalCost)
        {
            ...
            _fontStyle = FontFactory.GetFont("Tahoma", 11f, 1);
            _pdfPCell = new PdfPCell(new Phrase("List Of Package Cancellation Details", _fontStyle));
            _pdfPCell.Colspan = _totalColumn;
        ...
        private void ReportBody(List<HsBookingPdfModel> hsBookingPdfModel)
        {
            ...
            _pdfPCell = new PdfPCell(new Phrase("Package Name", _fontStyle));
            _pdfPCell.HorizontalAlignment = Element.ALIGN_CENTER;
            ...
            _pdfPCell = new PdfPCell(new Phrase("Package From", _fontStyle));
            _pdfPCell.HorizontalAlignment = Element.ALIGN_CENTER;
            _pdfPCell.VerticalAlignment = Element.ALIGN_MIDDLE;
            _pdfPCell.BackgroundColor = BaseColor.LightGray;
            _pdfTable.AddCell(_pdfPCell);

            _pdfPCell = new PdfPCell(new Phrase("Package To", _fontStyle));
            _pdfPCell.HorizontalAlignment = Element.ALIGN_CENTER;
            _pdfPCell.VerticalAlignment = Element.ALIGN_MIDDLE;
            _pdfPCell.BackgroundColor = BaseColor.LightGray;
            _pdfTable.AddCell(_pdfPCell);
