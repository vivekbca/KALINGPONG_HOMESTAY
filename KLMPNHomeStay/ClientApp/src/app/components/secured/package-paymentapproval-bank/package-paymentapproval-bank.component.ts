...
@Component({
  selector: 'app-package-paymentapproval-bank',
  templateUrl: './package-paymentapproval-bank.component.html',
  styleUrls: ['./package-paymentapproval-bank.component.css']
})
export class PackagePaymentapprovalBankComponent implements OnInit {
  packageList: any = [];
  packageDetail: any = [];
  p: number = 1;
  ...
  getAllPayPackageList() {
    this.tourService.getAllPaymentApprovalTourBank().subscribe(resp => {
  ...
  approveBookingPay(bookingId: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.tourService.approveBookingPaymentBank(packageComObj).subscribe(resp => {
